...
SchedulingFunctionSet* SimpleRISCV_H_nfw_NoBrPred_SchedulingFunctionSet = new SchedulingFunctionSet("SimpleRISCV_H_nfw_NoBrPred_SchedulingFunctionSet");

static SchedulingFunction *schedulingFunction_add = new SchedulingFunction(
  SimpleRISCV_H_nfw_NoBrPred_SchedulingFunctionSet,
  "add",
  0,
  [](PerformanceModel* perfModel_){
  SimpleRISCV_H_nfw_NoBrPred_PerformanceModel* perfModel = static_cast<SimpleRISCV_H_nfw_NoBrPred_PerformanceModel*>(perfModel_);
  // Enter
uint64_t n_Enter = perfModel->IF_stage;
// IPort_R
uint64_t n_IPort_R;
uint64_t n_IPort_R_max;
n_IPort_R_max = std::max({n_Enter, perfModel->noBranchPredModel.getPc()});
n_IPort_R = n_IPort_R_max + perfModel->iMemModel.getDelay();
// PC_Gen
uint64_t n_PC_Gen;
uint64_t n_PC_Gen_max;
n_PC_Gen_max = std::max({n_Enter, perfModel->noBranchPredModel.getPc()});
n_PC_Gen = n_PC_Gen_max + 1;
perfModel->noBranchPredModel.setPc_p(n_PC_Gen);
// IF_stage
uint64_t n_IF_stage;
...
uint64_t n_ALU;
n_ALU = n_ID_stage + 1;
perfModel->noBranchPredModel.setPc_np(n_ALU);
// EX_stage
uint64_t n_EX_stage = n_ALU;
perfModel->EX_stage = n_EX_stage;
